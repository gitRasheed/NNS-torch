...
def _reduce(x: torch.Tensor, dim=None, keepdim: bool = False, reduction: str = "mean") -> torch.Tensor:
    if reduction == "none":
        return x
    if reduction == "sum":
...
def lpm_reference(x, target=0.0, degree=1.0, dim=None, keepdim: bool = False, reduction: str = "mean", eps: float = EPS):
    return _reduce(lpm_elem_reference(x, target, degree, eps), dim, keepdim, reduction)


def upm_reference(x, target=0.0, degree=1.0, dim=None, keepdim: bool = False, reduction: str = "mean", eps: float = EPS):
    return _reduce(upm_elem_reference(x, target, degree, eps), dim, keepdim, reduction)
